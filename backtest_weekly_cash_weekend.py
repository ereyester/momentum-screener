...
def main():
    ...
    print(Fore.YELLOW + "========================================================")
    print("   Momentum Chimp Backtester (Cash Weekend) v1.0")
    print("   Score on Friday Close, Buy on Monday Open, Sell on Friday Close")
    print("========================================================\n" + Style.RESET_ALL)
    ...
    for i in range(start_idx, len(days) - 5, 5):
        calc_day = days[i]
        buy_day = days[i+1] # 翌営業日（月曜日の寄り付きで買う）
        sell_day = days[i+5] # 次の金曜日（金曜日の大引けで売る）
        ...
        for t in deck_tickers:
            if t in open_df.columns and t in close_df.columns:
                p_in = open_df[t].loc[buy_day]      # 月曜の始値
                p_out = close_df[t].loc[sell_day]   # 金曜の終値
        ...
        bm_return = 0.0
        if bm_ticker in open_df.columns and bm_ticker in close_df.columns:
            bm_in = open_df[bm_ticker].loc[buy_day]
            bm_out = close_df[bm_ticker].loc[sell_day]
            if pd.notna(bm_in) and pd.notna(bm_out) and bm_in > 0:
                bm_return = (bm_out / bm_in) - 1.0
                
        results.append({
            "Trade_Date": sell_day.strftime("%Y-%m-%d"),
            "PF_Return": pf_return,
            "BM_Return": bm_return
        })
    ...
    print("\n====================================================================================================")
    print("  🐒 バックテスト月次集計 (週末現金化: 月曜寄り買い/金曜引け売り)")
    print("====================================================================================================")
    ...
    print("\n========================================================")
    print(Fore.CYAN + "  🏆 バックテスト最終結果 (Cash Weekend)" + Style.RESET_ALL)
    print("========================================================")
