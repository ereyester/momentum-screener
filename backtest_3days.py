...
def main():
    ...
    print(Fore.YELLOW + "========================================================")
    print("   Momentum Chimp Backtester (3-Days Open-to-Open) v1.0")
    print("   3-Day Rebalancing, No Stop-Loss, Past 2 Years")
    print("========================================================\n" + Style.RESET_ALL)
    ...
    for i in range(start_idx, len(days) - 3, 3):
        trade_day = days[i]
        next_trade_day = days[i+3]
        ...
        for ticker in tickers:
            ...
            # i日目の寄り付きで買うため、スコア計算に使えるのは前日(i-1)の大引けデータまで
        ...
        for t in deck_tickers:
            if t in open_df.columns:
                p_in = open_df[t].loc[trade_day]
                p_out = open_df[t].loc[next_trade_day]
        ...
        bm_return = 0.0
        if bm_ticker in open_df.columns:
            bm_in = open_df[bm_ticker].loc[trade_day]
            bm_out = open_df[bm_ticker].loc[next_trade_day]
    ...
    # 月ごとにサマリーを作成して表示（160行以上になるため）
    res_df["Month"] = pd.to_datetime(res_df["Trade_Date"]).dt.to_period('M')
    
    # 各月の最初と最後のCum値を取って月間リターンを計算
    monthly_summary = []
    for month, group in res_df.groupby("Month"):
        start_pf = group["PF_Cum"].iloc[0] / (1.0 + group["PF_Return"].iloc[0]) # 月初の前の状態
    ...
    print("\n====================================================================================================")
    print("  🐒 バックテスト月次集計 (3営業日ごと・寄り付き売買 / ロスカットなし)")
    print("====================================================================================================")
    ...
    print("\n========================================================")
    print(Fore.CYAN + "  🏆 バックテスト最終結果 (3-Days Open-to-Open)" + Style.RESET_ALL)
    print("========================================================")
    ...
    if final_pf > final_bm:
        ...
        print(Fore.RED + f"  💀 チンパンはベンチマークに {underperform:+.2f}% 負けました（往復ビンタのノイズ過多）" + Style.RESET_ALL)
